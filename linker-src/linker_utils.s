...
inst_needs_relocation:
    	...
    	srl $a0 $a0 26 # Shift the instruction 26 right to isolate the opcode
    	addiu $t1 $0 2
    	beq $a0 $t1 inst_needsTrue # Check if the opcode is 2 (j)
    	addiu $t1 $0 3
    	beq $a0 $t1 inst_needsTrue # Check if the opcode is 3 (jal)
   		addiu $v0 $0 0 # If the opcode wasn't 2 or 3, return 0
    	jr $ra
...
relocate_inst:
	addiu $sp $sp -16
    	...
    	addiu $a0 $a3 0  # Get the Symbol name from the relocation table based on the offset
     	jal symbol_for_addr
    	beq $v0 $0 relocate_instError
            
        addiu $a0 $s1 0
    	addiu $a1 $v0 0 # Get the symbol address fro the symbol table based on the name 
    	jal addr_for_symbol
   		addiu $t0 $0, -1 # Error if the symbol doesn't exist
    	beq $v0 $t0 relocate_instError
