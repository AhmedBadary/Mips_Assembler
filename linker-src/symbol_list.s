...
addr_forLoop:
	...
	lw $a0 4($a0) # Check if the current name is the name we're looking for
	jal streq
	
	move $a0 $s0
	move $a1 $s1
	
	beq $v0 $0 addr_forEnd
	
	lw $t1 8($a0) # Check if we are at the end of the list
	beq $t1 $0 addr_forError	

	add $a0 $0 $t1 # Move to the next node
	j addr_forLoop
	
addr_forEnd:
	lw $v0 0($a0) # Copy in the address of the node
	lw $ra 0($sp)
	lw $s0 4($sp)
	lw $s1 8($sp)
	addiu $sp $sp 12
